...
from dataset.generate_dataset_HomographyRegNet_v1 import generate_points, warp, pack
...
parser.add_argument('--model_stage_5', type=str)
parser.add_argument('--norm_value_stage_5', type=int)
...
pts_norm_value_stage_5 = (args.norm_value_stage_5, args.norm_value_stage_5) #(8, 8)


def loader(data_dir, num_samples_per_image):
    num_image_count = 0
    for image_path in glob.iglob(os.path.join(data_dir, '*.jpg')):
        if args.num_images is not None and num_image_count >= args.num_images:
            return
...
def process_image(img_pair, pts_in_orig, pts_in_warp, img_orig, pts_in_orig_nonrs, model_dict):
    ...
    #           outputs: (8,) normalized 4 pts in resized original image, absolute coordiante
    #####
    ...
    #errs = np.mean(np.sqrt(np.sum(np.square(pts_in_orig - pts_pred_1_rs), -1)))
    #err_dict['error_stage_1'] = errs.tolist()
    errs_nonrs = np.mean(np.sqrt(np.sum(np.square(pts_in_orig_nonrs - pts_pred_1_nonrs), -1)))
    err_dict['error_stage_1_nonrs'] = errs_nonrs
    ...
    pts_gt_1_norm = pts_gt_1 / np.array(pts_norm_value_stage_2).reshape(-1, 2)

    # for generate next stage dataset
    img_pair_new = img_pair_1
    pts_new_norm = pts_gt_1_norm

    # 预测的四个角点的偏移不要超过 1/2 target_size, 否则预测的点顺序会错位
    #if np.max(np.abs(pts_new_norm[:, 0])) >= 0.5 or np.max(np.abs(pts_new_norm[:, 1])) >= 0.5: # for norm_value_128
    if ( np.max(np.abs(pts_new_norm[:, 0])) * pts_norm_value_stage_2[0] >= target_size[0]//2 or 
            np.max(np.abs(pts_new_norm[:, 1])) * pts_norm_value_stage_2[1] >= target_size[1]//2 ):
        print('skip')
        flag = False
    ...
    if args.num_models >= 2:
        ...
        errs_nonrs = np.mean(np.sqrt(np.sum(np.square(pts_in_orig_nonrs - pts_pred_2_nonrs), -1)))
        err_dict['error_stage_2_nonrs'] = errs_nonrs
        ...
        pts_gt_2_norm = pts_gt_2 / np.array(pts_norm_value_stage_3).reshape(-1, 2)
        ...
        if ( np.max(np.abs(pts_new_norm[:, 0])) * pts_norm_value_stage_3[0] >= target_size[0]//2 or 
                np.max(np.abs(pts_new_norm[:, 1])) * pts_norm_value_stage_3[1] >= target_size[1]//2 ):
            print('skip')
            flag = False
    ...
    if args.num_models >= 3:
        ...
        err_dict['error_stage_3_nonrs'] = errs_nonrs
        ...
        pts_gt_3_norm = pts_gt_3 / np.array(pts_norm_value_stage_4).reshape(-1, 2)

        # for generate next stage dataset
        img_pair_new = img_pair_3
        pts_new_norm = pts_gt_3_norm

        if ( np.max(np.abs(pts_new_norm[:, 0])) * pts_norm_value_stage_4[0] >= target_size[0]//2 or 
                np.max(np.abs(pts_new_norm[:, 1])) * pts_norm_value_stage_4[1] >= target_size[1]//2 ):
            print('skip')
            flag = False
    ...
    if args.num_models >= 4:
        ...
        err_dict['error_stage_4_nonrs'] = errs_nonrs
        ...
        pts_gt_4_norm = pts_gt_4 / np.array(pts_norm_value_stage_5).reshape(-1, 2)

        # for generate next stage dataset
        img_pair_new = img_pair_4
        pts_new_norm = pts_gt_4_norm

        if ( np.max(np.abs(pts_new_norm[:, 0])) * pts_norm_value_stage_5[0] >= target_size[0]//2 or 
                np.max(np.abs(pts_new_norm[:, 1])) * pts_norm_value_stage_5[1] >= target_size[1]//2 ):
            print('skip')
            flag = False
...
def main():
    ...
    for img_pair, pts_in_orig, pts_in_warp, img_orig, pts_in_orig_nonrs in data_loader:
        ...
        error_stage_1_nonrs.append(err_dict['error_stage_1_nonrs'])
        if 'error_stage_2_nonrs' in err_dict:
            error_stage_2_nonrs.append(err_dict['error_stage_2_nonrs'])
        if 'error_stage_3_nonrs' in err_dict:
            error_stage_3_nonrs.append(err_dict['error_stage_3_nonrs'])
        if 'error_stage_4_nonrs' in err_dict:
            error_stage_4_nonrs.append(err_dict['error_stage_4_nonrs'])
    ...
    print('*'*80)
    print('stage_1 | mean pts error in origin image: {:.2f} [pixel]'.format(np.mean(error_stage_1_nonrs)))
    print('          number samples: {}'.format(len(error_stage_1_nonrs)))
    if args.num_models >= 2:
        print('stage_2 | mean pts error in origin image: {:.2f} [pixel]'.format(np.mean(error_stage_2_nonrs)))
        print('          number samples: {}'.format(len(error_stage_2_nonrs)))
    if args.num_models >= 3:
        print('stage_3 | mean pts error in origin image: {:.2f} [pixel]'.format(np.mean(error_stage_3_nonrs)))
        print('          number samples: {}'.format(len(error_stage_3_nonrs)))
    if args.num_models >= 4:
        print('stage_4 | mean pts error in origin image: {:.2f} [pixel]'.format(np.mean(error_stage_4_nonrs)))
        print('          number samples: {}'.format(len(error_stage_4_nonrs)))
